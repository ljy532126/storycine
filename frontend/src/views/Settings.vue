    ...
    <div v-show="settingsTab === 'image'" class="st-section">
      <div class="st-grid">
        ...
        <div class="st-card">
          ...
          <div class="st-toggle-row"><div class="st-toggle-info"><span class="st-toggle-label">视频风格化模式</span><span class="st-toggle-hint">生图时注入彩铅艺术风格绕过AI审查；生视频时注入写实电影级风格还原真实皮肤质感；动漫风格自动跳过</span></div><el-switch v-model="imgCfg.characterStyleMode" @change="saveImgCfg" /></div>
        </div>
      ...
      <div class="st-grid">
        <div class="st-card">
          ...
          <template v-if="stor.enabled">
            <!-- 隐藏输入吸收浏览器自动填充，防止填入真实字段 -->
            <input type="text" style="position:absolute;left:-9999px;width:1px;height:1px" autocomplete="username" tabindex="-1" />
            <input type="password" style="position:absolute;left:-9999px;width:1px;height:1px" autocomplete="current-password" tabindex="-1" />
            <div class="st-field-row"><span class="st-field-label">AccessKey ID</span><el-input v-model="stor.accessKeyId" size="small" style="width:320px" placeholder="阿里云/腾讯云 AccessKey ID" autocomplete="new-username" /></div>
            <div class="st-field-row"><span class="st-field-label">AccessKey Secret</span><el-input v-model="stor.accessKeySecret" size="small" style="width:320px" type="password" show-password :placeholder="stor._hasSecret ? '已保存（留空不修改）' : '密钥'" autocomplete="new-password" /></div>
            ...
            <div class="pf-edit-row" v-if="smsEnabled">
              ...
              <div class="pf-edit-val">
                <div v-if="!profileUser.phone" class="pf-phone-bind">
                  ...
                  <div class="pf-phone-fields">
                    <div class="input-counter-wrap" style="flex:1"><el-input v-model="phoneBindForm.code" placeholder="短信验证码" maxlength="6" /><span v-if="phoneBindForm.code" class="input-counter">{{ phoneBindForm.code.length }}/6</span></div>
                    <el-button @click="sendBindSms" :loading="bindSending" :disabled="bindCooldown > 0 || !phoneBindForm.phone" style="min-width:110px;height:40px">{{ bindCooldown > 0 ? bindCooldown + 's' : '获取验证码' }}</el-button>
                  </div>
                  <el-button type="primary" @click="bindPhone" :loading="bindSubmitting" :disabled="!phoneBindForm.phone || !phoneBindForm.code" size="large">确认绑定</el-button>
                </div>
