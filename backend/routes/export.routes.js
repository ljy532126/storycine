...
function buildHTML(name, date, scripts, sbs, types) {
  ...
  if (types.includes('script')) { h += '<h2>📝 剧本全文</h2>'; scripts.forEach(ep => { h += `<h3>第${ep.episodeNumber}集 ${ep.episodeTitle||''}</h3>${ep.summary?`<p>${ep.summary}</p>`:''}`; (ep.scenes||[]).forEach(s => { h += `<div class="card"><strong>场次${s.sceneNumber} — ${s.location||''} · ${s.timeOfDay||''}${s.atmosphere?'（'+s.atmosphere+'）':''}</strong>${s.sceneDescription?`<p>📝 描述：${s.sceneDescription}</p>`:''}${s.notes?`<p>📌 备注：${s.notes}</p>`:''}`; (s.dialogues||[]).forEach(d => h += `<p style="margin-left:16px"><strong>${d.characterName}</strong>：${d.text}${d.actionHint?'<br><small>🎭 '+d.actionHint+'</small>':''}${d.cameraHint?'<br><small>🎥 '+d.cameraHint+'</small>':''}${d.innerThought?'<br><small>💭 '+d.innerThought+'</small>':''}</p>`); h += '</div>'; }); }); }
  if (types.includes('shots')) { h += '<h2>🎬 分镜全文</h2>'; scripts.forEach(ep => { h += `<h3>第${ep.episodeNumber}集</h3>`; (ep.scenes||[]).forEach(s => { h += `<div class="card"><strong>镜号${s.sceneNumber}</strong> <span class="tag">${s.shotType||'中景'}</span> <span class="tag">${s.cameraMovement||''}</span> <span class="tag">${s.duration||3}s</span><br>📍 ${s.location||''} · ⏰ ${s.timeOfDay||''}${s.lighting?`<br>💡 光影：${s.lighting}`:''}${s.sceneDescription?`<br>📝 描述：${s.sceneDescription}`:''}${s.notes?`<br>📌 备注：${s.notes}`:''}`; (s.dialogues||[]).forEach(d => h += `<br><strong>${d.characterName}</strong>："${d.text}"${d.actionHint?' <small>('+d.actionHint+')</small>':''}${d.cameraHint?' <small>【'+d.cameraHint+'】</small>':''}`); h += '</div>'; }); }); }
...
function buildMarkdown(name, date, scripts, sbs, types) {
  let md = `# ${name}\n导出日期：${date}\n\n`;
  if (types.includes('script')) { md += '## 📝 剧本全文\n\n'; scripts.forEach(ep => { md += `### 第${ep.episodeNumber}集 ${ep.episodeTitle||''}\n${ep.summary?ep.summary+'\n\n':'\n'}`; (ep.scenes||[]).forEach(s => { md += `**场次${s.sceneNumber}** — ${s.location||''} · ${s.timeOfDay||''}${s.atmosphere?'（'+s.atmosphere+'）':''}\n${s.sceneDescription?'> 描述：'+s.sceneDescription+'\n':''}${s.notes?'> 备注：'+s.notes+'\n':''}\n`; (s.dialogues||[]).forEach(d => md += `- **${d.characterName}**：${d.text} ${d.actionHint?`_(${d.actionHint})_`:''}${d.cameraHint?`\n  ↳ 镜头：${d.cameraHint}`:''}${d.innerThought?`\n  ↳ 内心：${d.innerThought}`:''}\n`); md += '\n'; }); }); }
  if (types.includes('shots')) { md += '## 🎬 分镜全文\n\n'; scripts.forEach(ep => { md += `### 第${ep.episodeNumber}集\n\n`; (ep.scenes||[]).forEach(s => { md += `**镜号${s.sceneNumber}** | ${s.shotType||'中景'} | ${s.cameraMovement||'静止'} | ${s.duration||3}s\n- 场景：${s.location||''} | 时间：${s.timeOfDay||''}${s.lighting?` | 光影：${s.lighting}`:''}\n${s.sceneDescription?`- 描述：${s.sceneDescription}\n`:''}${s.notes?`- 备注：${s.notes}\n`:''}`; (s.dialogues||[]).forEach(d => md += `- **${d.characterName}**：${d.text}${d.actionHint?` (${d.actionHint})`:''}${d.cameraHint?` 【${d.cameraHint}】`:''}\n`); md += '\n'; }); }); }
  if (types.includes('full_storyboard')) { md += '## 🎞️ 故事板\n\n'; sbs.forEach(sb => { md += `### ${sb.totalShots}镜头 · ${sb.totalDuration}s\n\n`; (sb.shots||[]).forEach(s => md += `- **镜头${s.shotNumber}** ${s.shotType} ${s.cameraMovement} ${s.duration}s\n  ${s.imageDescription||''}${s.dialogue?.text?`\n  **${s.dialogue.characterName}**："${s.dialogue.text}"`:''}\n`); }); }
  return md + `\n---\nStoryCine · ${name} · ${date}`;
}
...
function buildCSV(name, scripts, sbs, types) {
  ...
  if (types.includes('shots')) {
    csv += '集数,镜号,场景,时间,景别,构图,运镜,光影,时长(s),分镜描述,备注,角色,台词,动作提示,镜头提示,内心独白\n';
    scripts.forEach(ep => { (ep.scenes||[]).forEach(s => { const desc=(s.sceneDescription||'').replace(/"/g,'""'); const notes=(s.notes||'').replace(/"/g,'""'); if ((s.dialogues||[]).length > 0) { s.dialogues.forEach(d => csv += `${ep.episodeNumber},${s.sceneNumber},"${s.location||''}","${s.timeOfDay||''}",${s.shotType||''},${s.composition||''},${s.cameraMovement||''},${s.lighting||''},${s.duration||''},"${desc}","${notes}","${d.characterName||''}","${(d.text||'').replace(/"/g,'""')}","${d.actionHint||''}","${d.cameraHint||''}","${(d.innerThought||'').replace(/"/g,'""')}"\n`); } else { csv += `${ep.episodeNumber},${s.sceneNumber},"${s.location||''}","${s.timeOfDay||''}",${s.shotType||''},${s.composition||''},${s.cameraMovement||''},${s.lighting||''},${s.duration||''},"${desc}","${notes}","","","","",""\n`; } }); });
  }
